...
// 验证密钥长度
void ChaCha20Poly1305_Decryptor::validate_key(const std::vector<uint8_t>& key) {
    if (key.size() != 32) {
...
// 提取Nonce
std::vector<uint8_t> ChaCha20Poly1305_Decryptor::extract_nonce(
    ...
    return std::vector<uint8_t>(ciphertext.begin(), ciphertext.begin() + nonce_size);
}

// 提取认证标签
std::vector<uint8_t> ChaCha20Poly1305_Decryptor::extract_tag(
    const std::vector<uint8_t>& ciphertext,
    size_t tag_size
) {
...
// 提取实际密文
std::vector<uint8_t> ChaCha20Poly1305_Decryptor::extract_actual_ciphertext(
    const std::vector<uint8_t>& ciphertext,
    size_t nonce_size,
    size_t tag_size
) {
...
// 生成解密文件名
std::string ChaCha20Poly1305_Decryptor::generate_decrypted_filename(const std::string& encrypted_path) {
    size_t last_dot = encrypted_path.find_last_of('.');
    if (last_dot == std::string::npos) {
        return encrypted_path + ".decrypted";
    }

    std::string path = encrypted_path.substr(0, last_dot);
...
// 解密内存中数据
std::vector<uint8_t> ChaCha20Poly1305_Decryptor::decrypt_data(
    const std::vector<uint8_t>& ciphertext,
    ...
    std::vector<uint8_t> nonce = extract_nonce(ciphertext, nonce_size);
    std::vector<uint8_t> tag = extract_tag(ciphertext, tag_size);
    std::vector<uint8_t> actual_ciphertext = extract_actual_ciphertext(ciphertext, nonce_size, tag_size);
    ...
    // 初始化解密操作 - 使用ChaCha20-Poly1305
    if (EVP_DecryptInit_ex(ctx, EVP_chacha20_poly1305(), nullptr, nullptr, nullptr) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        throw std::runtime_error("Failed to initialize ChaCha20-Poly1305 decryption: " + get_openssl_error());
    }

    // 设置Nonce长度
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, static_cast<int>(nonce_size), nullptr) != 1) {
        EVP_CIPHER_CTX_free(ctx);
    ...
    // 设置认证标签
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, static_cast<int>(tag_size), tag.data()) != 1) {
        EVP_CIPHER_CTX_free(ctx);
...
// 解密文件
std::vector<uint8_t> ChaCha20Poly1305_Decryptor::decrypt_file(
    ...
    std::cout << "File decrypted successfully with ChaCha20-Poly1305: " << input_path
